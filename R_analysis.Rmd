...
HiFiC1 <- filter(data_clean_HiFiCas9, Condition == "Control1") %>% group_by(Number) %>% select(X.Indels.HiFiCas9, Number, ID, GUARDWithG)
HiFiC2 <- filter(data_clean_HiFiCas9, Condition == "Control2") %>% group_by(Number) %>% select(X.Indels.HiFiCas9, Number, ID, GUARDWithG)
...
HiFiD1 <- filter(data_clean_HiFiCas9, Condition == "Dox.1") %>% group_by(Number) %>% select(X.Indels.HiFiCas9, ID, Number)
HiFiD2 <- filter(data_clean_HiFiCas9, Condition == "Dox.2") %>% group_by(Number) %>% select(X.Indels.HiFiCas9, ID, Number)
...
annotate("text", x = 1.5, y = 20, label = c("0.9964")) +
annotate("segment", x = 1.1, xend = 1.9, y = 19.4, yend = 19.4, alpha = 0.7) +
coord_cartesian(ylim=c(0, 20)) +
scale_x_discrete(labels=c("FALSE" = "GUARD RNAs", "TRUE" = "NT controls")) +
theme(axis.text.x = element_text(size = rel(1.2)))
...
theme_bw() +
geom_jitter(alpha=0.1, size = 2, width = 0.2) +
labs(title= "HiFi-Cas9: PAM overlap", x=NULL, y="Overlap (bp)") +
...
group_by(Number, ID, Condition, Functional, Perfect) %>%
filter(Condition == "Dox.1" & NT == "FALSE") %>%
tally() %>%
count(n)

data_clean_HiFiCas9_split %>%
group_by(Number, ID, Condition, Guide, Functional, NT, mean_NT, Perfect, X.Indels.HiFiCas9) %>%
filter(Functional == "TRUE") %>%
filter(Condition == "Dox.1"& NT == "FALSE") %>%
tally() %>%
count(n)
...
geom_jitter(size=1.5, alpha=0.8, width = 0.1, height = 0.1) + labs(title= "Cas9: Off-target", x=NULL, y="Indels (%)") +
scale_x_discrete(labels=c("FALSE" = "GUARD RNAs", "TRUE" = "NT controls"))+
facet_wrap(~ID, scales ='free_y')+
theme(axis.text.x = element_text(angle = 90)) +
geom_text_repel(aes(label=ifelse(GUARDWithG=="GTAGATGGTTGGGTAG", "GUARD v2", "")), nudge_y=0.5, nudge_x =0.3) +
geom_text_repel(aes(label=ifelse(GUARDWithG=="GTAAGCAGAAGAAGAA", "GUARD v1", "")), nudge_y=0.5, nudge_x =0.3) +
theme(axis.text.x = element_text(size = rel(1.2)))
